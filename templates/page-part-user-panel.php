<?php 
  //Added by Lookthink to handle Operational Hours
  //Works with advanced custom fields from the front page
  $frontpage_ID = get_option('page_on_front');
  $day = date("l");
  $check = get_field( "show_opening_hours", $frontpage_ID);
  $output = '<span class="openHours">';
  if($check){
    $output .= '<span class="openHours__title">Open Today:</span><br />';
    switch($day){
      case 'Monday':
        $output .= get_field( "monday", $frontpage_ID );
        break;
      case 'Tuesday':
        $output .= get_field( "tuesday", $frontpage_ID );
        break;
      case 'Wednesday':
        $output .= get_field( "wednesday", $frontpage_ID );
        break;
      case 'Thursday':
        $output .= get_field( "thursday", $frontpage_ID );
        break;
      case 'Friday':
        $output .= get_field( "friday", $frontpage_ID );
        break;
      case 'Saturday':
        $output .= get_field( "saturday", $frontpage_ID );
        break;
      case 'Sunday':
        $output .= get_field( "sunday", $frontpage_ID );
        break;
    }
  }
  $output .= "</span>";
